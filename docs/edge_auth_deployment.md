...
10. **Minimal Private Health**: Public `/api/health` is still protected by Access. The loopback-only `/api/health/internal` route is reserved for the container health probe and returns only minimal status (`{"status": "ok", "service": "job-applier"}`); neither route exposes internal configuration.
