...
# Allowed safe environment variables necessary for Chromium and X11
...
# Patterns that indicate sensitive secrets to always strip (case-insensitive)
...
def get_sanitized_browser_env(
    base_env: dict[str, str] | None = None,
    extra_allowed: set[str] | None = None,
) -> dict[str, str | float | bool]:
    """
    Returns an environment dictionary stripped of sensitive credentials, API keys,
    tokens, and internal secrets before launching the browser or child processes.
    Only essential system and display environment variables are preserved.
    """
    source = dict(os.environ if base_env is None else base_env)
    allowed = set(SAFE_ENV_VARS)
    if extra_allowed:
    ...
    for k, v in source.items():
        ...
        # If it matches any secret pattern, strictly strip it
        if any(pat in k_upper for pat in SECRET_ENV_PATTERNS):
            continue

        # Unknown variables without dangerous patterns are omitted by default for defense in depth

    # If using ambient environment and HOME is unconfigured or non-writable, default to /tmp
    if base_env is None:
        home_val = str(sanitized.get("HOME", "")).strip()
        if not home_val or not os.access(home_val, os.W_OK):
            sanitized["HOME"] = "/tmp"
